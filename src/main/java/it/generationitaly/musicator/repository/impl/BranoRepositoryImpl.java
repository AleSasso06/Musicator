...
public class BranoRepositoryImpl extends JpaRepositoryImpl<Brano, Long> implements BranoRepository{
	...
	public List<Brano> findByArtista(String artista) {
		...
		try {
			...
			TypedQuery<Brano> query = em.createQuery("FROM album_brano ab JOIN brano b ON ab.brano_id = b.id JOIN album al ON al.id = ab.album_id JOIN artista ar ON al.artista_id = ar.id WHERE ar.id LIKE CONCAT('%',:artista ,'%')", Brano.class);
	...
	public List<Brano> findByAlbum(String album) {
		...
		try {
			...
			tx = em.getTransaction();
			tx.begin();
			// SELECT * FROM album_brano JOIN brano ON album_brano.brano_id = brano.id JOIN album ON album.id = album_brano.album_id;
			TypedQuery<Brano> query = em.createQuery("FROM album_brano ab JOIN brano b ON ab.brano_id = b.id JOIN album a ON a.id = ab.album_id WHERE a.titolo LIKE CONCAT('%',:album ,'%')", Brano.class);
	...
	public List<Brano> findByGenere(String genere) {
		...
		try {
			...
			TypedQuery<Brano> query = em.createQuery("FROM brano b JOIN genere g ON b.genere_id = g.id WHERE g.nome LIKE CONCAT('%',:genere ,'%')", Brano.class);
	...
	public List<Brano> findByLingua(String lingua) {
		...
		try {
			...
			tx.begin();
			// test like
			TypedQuery<Brano> query = em.createQuery("FROM Brano b WHERE b.lingua LIKE CONCAT('%',:lingua ,'%')", Brano.class);
